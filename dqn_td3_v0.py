...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with T.no_grad():
            noise = (
                T.FloatTensor(action).data.normal_(0, self.policy_noise).to(device)
...
class JointAgent:
    ...
    def train(self):
        if len(self.replay_buffer.storage) > 256:
            ...
            q_target = self.dqn.forward(T.tensor(next_states, dtype=T.float32, device=device)).detach()
            dones = T.tensor(dones, dtype=T.float32, device=device)
            rewards = T.from_numpy(rewards).float().to(device)  # convert numpy array to tensor and ensure it's float32  
            q_target_val = rewards + (1.0 - dones) * q_target.max(1)[0].unsqueeze(1)

            q_pred = q_pred.gather(1, discrete_actions.unsqueeze(1)).squeeze(1)
            dqn_loss = self.dqn.loss(q_pred, q_target_val.squeeze(1))

            self.dqn.optimizer.zero_grad()
            dqn_loss = dqn_loss.float()
            dqn_loss.backward()
    ...
    def get_weights(self):
        return (self.dqn.state_dict(), self.td3.actor.state_dict(), self.td3.critic.state_dict())

    def set_weights(self, weights):
        self.dqn.load_state_dict(weights[0])
        self.td3.actor.load_state_dict(weights[1])
        self.td3.critic.load_state_dict(weights[2])
