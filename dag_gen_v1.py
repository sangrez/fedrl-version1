...
# THIS FILE IS USED TO GENERATE THE DATASETS FOR TRAINING AND TESTING AND EACH DAG DATASET CONTAINS 70% OF THE SPECIFIC DAG TYPE AND 30% OF OTHER DAG TYPES
...
def create_weighted_dag_datasets(num_dags, task_num, data_ranges, dag_types, train_ratio=0.8, majority_ratio=0.7):
    """
    Create datasets where 70% of the DAGs are of a specific type, and the remaining 30% are of other types.
    """
    all_datasets = {dag_type: [] for dag_type in dag_types}
    train_datasets = {dag_type: [] for dag_type in dag_types}
    test_dataset = []

    # Generate majority-minority split datasets for each DAG type
    for dag_type in dag_types:
        ...
        # Generate majority DAGs of the specific type
        majority_dags = []
        ...
        # Generate minority DAGs from other types
        minority_dags = []
        ...
        # Combine majority and minority DAGs and shuffle them
        dataset = majority_dags + minority_dags
        random.shuffle(dataset)

        # Add to the all_datasets dictionary
        all_datasets[dag_type] = dataset

    # Split into train and test datasets for each type
    for dag_type, dags in all_datasets.items():
        random.shuffle(dags)  # Shuffle the dataset
...
if __name__ == '__main__':
    # Configuration
    NUM_DAGS = 5000  # Number of DAGs per type (100 for train, 25 for test)
    ...
    DATA_RANGES = {
        'linear': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'branching': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'mixed': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'grid': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'fork-join': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'star': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'tree': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
        'cycle-free-mesh': {'data_size_range': (125, 125), 'workload_range': (100, 100)},
    }

    DAG_TYPES = list(DATA_RANGES.keys())

    # Create and split datasets
    train_datasets, test_dataset = create_weighted_dag_datasets(
        num_dags=NUM_DAGS,
        task_num=TASK_NUM,
